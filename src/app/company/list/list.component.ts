...
export class CompanyListComponent implements OnInit {
  ...
  searchText(){
    this.spinner.show();
    setTimeout(()=>{            
      this.companyList =this.companyList.filter(u=>u.EmailId.toLowerCase().indexOf(this.companyForm.value.searchtext.toLowerCase())>=0 || u.CompanyName.toLowerCase().indexOf(this.companyForm.value.searchtext.toLowerCase())>=0);
      this.spinner.hide();  
    });
  }
